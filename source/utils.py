...
def split_data(data, test_size=0.2):
    unique_dates = np.sort(data.start_time.dt.date.unique())
    num_days_test = int(len(unique_dates) * test_size)
    test_dates = unique_dates[-num_days_test:]
    test_data = data[data.start_time.dt.date.isin(test_dates)]
    train_data = data[~data.start_time.dt.date.isin(test_dates)]
