...
class LastNearestOrder(Policy):
    ...
    def take_action(self, state: State):
        ...
        return self.get_current_nearest_order(orders, state.distance_map)

    def get_current_nearest_order(self, orders, couriers, distance_map):
        if not orders:
            return {
        ...
        for j, courier in enumerate(couriers):
            ...
            for i, order in enumerate(orders):
                ...
                d = self.distance_map[(courier_lat, courier_lng)][(order_lat, order_lng)]
                if d < nearest_order[j]['distance']:
                    move_lat, move_lng = self.compute_movement_location(courier_lat, courier_lng, order_lat, order_lng, d)
                    nearest_order[j] = {
                        'order_id': i,
                        'distance': d,
                        'lat': move_lat,
